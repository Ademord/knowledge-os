...
The returned pack carries its own change report (a `_merge` block: sources, consolidations with whys, bridging nodes, coverage added, rejections with reasons). On paste-back the app runs the full pack validation **plus total accounting** — any source topic that is neither alive, aliased, consolidated, nor rejected is a hard error naming it — then renders the report ("From 42 source topics: 31 preserved · 7 consolidated · 3 bridges added · 1 rejected") so you can audit the AI's structural judgments before importing. Imports are **non-destructive**: the merged graph becomes a new pack, sources stay untouched, and your progress carries over deterministically through ids, aliases, and the consolidation map.

**✎ Edit this graph** is the single-pack sibling — "add a prerequisite for softmax", "split this cluster" — same invariant, same accounting, same report. It imports as a new pack by default; for custom packs you can replace the original in place (progress carried, plan regenerated). Both operations complete the data story: every domain, built-in or custom, can be **downloaded or copied as JSON** from the switcher, and packs can be imported from files as well as pasted.
