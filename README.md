...
## Bring your own subject — "I want to learn Cooking"

The ＋ button (bottom-right of the Journey, or "＋ New learning path" in the domain switcher) turns any subject into a full domain pack **without writing JSON yourself**: type the subject → **📋 Copy Graph Author Prompt** → paste it into Claude/GPT/Gemini → your AI returns the whole subject as one JSON block → paste that back → the app validates it mechanically (unique ids, edge integrity, dependency cycles, practice data — the same checks as `validate.js`) → **Import & start**. Validation failures come with a **Copy Fix Prompt** that sends the exact errors back to your AI for a corrected pack. Imported packs live in your browser only (localStorage), appear in the domain switcher, run the full machine (curriculum drafting → dated plan → sessions → Analyzer), and can be downloaded as files to share or removed (with their progress) from the switcher. The switcher also carries **export/import progress** for moving your data between browsers.

### Learning priority — core · useful · detail · skip

Concept exists ≠ concept is on your path ≠ concept is scheduled tonight. Every unit carries a **pack-default priority** (`pr`, authored by the AI that built the pack, carried by export/merge/share) and you can **override it locally** per unit (the PRIORITY chips on any topic's card; effective = your override ?? pack default). The planner schedules **core + useful**; **detail** stays in the graph as a quiet *"Optional depth — unscheduled"* shelf on its cluster's exam night and workspace, one click to promote; **skip** is fully absent from the Journey (including review) while still existing in the graph. Graph Surgery changes pack defaults ("the authored structure is wrong"); your overrides are for "the structure is fine, I want to learn it differently" — and surgery can never silently defeat an override.

### Merging paths & graph surgery

**The governing division: your AI proposes → KnowledgeOS computes what structurally happened and validates it → you decide whether to apply.** The LLM has zero authority over state.

Multiple learning paths inevitably overlap — the German pack and a travel pack both teach ordering food; a cooking pack and a chemistry pack both touch heat transfer. **⧉ Merge & Expand packs** (in the domain switcher) applies the same copy/paste pattern to graph construction: pick 2+ packs (any mix of built-in and custom), optionally state an intent, and copy a merge prompt that hands your AI the complete source graphs plus one governing invariant, **mechanically enforced on import**:

> Never lose source knowledge silently. Every source topic must either survive (same id, or absorbed into a unit that lists it in its `al` alias array), be deliberately consolidated into another unit, or be explicitly rejected with a reason.

**✎ Graph surgery** is the single-pack sibling — reachable from the switcher or straight from a cluster's workspace ("add the missing prerequisites", "split this cluster", "this is too detailed for my goal"). The surgery prompt teaches the surgeon the **three layers** — existence, priority, schedule — so *"remove Softmax"* is interpreted, not obeyed: a real-but-over-detailed concept gets **demoted to `detail`** (recorded in `_merge.reclassified`) instead of deleted, and the surgeon states its reading in `_merge.interpretation` ("removing it would break the dependency chain, so I demoted it instead").

On paste-back the app runs full validation plus total accounting, then computes its **own deterministic diff from the entities** (never trusting the AI's report): added / removed / renamed-via-alias / reclassified / consolidated / dependency changes — plus a content-change detector, so a whole-pack prose rewrite can never hide behind a zero-row diff. Below it, **LEARNING IMPACT** classifies your live plan (`✓ 30 sessions unchanged · ⚠ Session 4 — prerequisite changed · ⚠ Session 7 — moved to optional depth`) on *effective* priority, so a topic you've pinned shows "keeps your override — Journey unchanged". Then **Cancel / Apply changes**.

**Apply is atomic and non-destructive.** Merges import as a new pack with sources untouched. Surgery on a custom pack can replace it in place — and your progress is *reconciled, never reset*: mastery, misconceptions, notes, timers, done stamps, todo checks and your plan are all carried across with ids remapped through aliases and consolidations (hub renames included). A failed write rolls everything back. The pack itself keeps an **intent ledger** (`_meta.surgeries`, written from the stored pack so the LLM can't author its own history). Afterwards the Journey is *marked as needing review* — "Your learning structure changed" with the impact report and **[review & update journey]** — and is never rewritten without that click; the update rebuilds future sessions while preserving all done history and anything you touched.

Both operations complete the data story: every domain, built-in or custom, can be **downloaded or copied as JSON** from the switcher, and packs can be imported from files as well as pasted.
...
**Explore** holds everything else. The galaxy is now primarily **the map**: clicking a cluster opens its **Cluster Workspace** — the constellation stays lit while the rest recedes, and the panel carries the three distinct concepts side by side: **Study** (the guided route), the six **AI tool** chips (Zoltraak · AI Tutor · Deep Dive · Cheatsheet · Quiz · Visualization — they understand and transform, never mutate the graph), and **Graph surgery** (modify the knowledge structure itself), plus the optional-depth shelf and the member list. Explore also keeps layers, the inspector, review coloring, rediscovery/compression, Analytics, and the ⚡ Zoltraak power modal (quietly apply a decomposition to your Journey's order, or hand-edit it — no approval ceremony anywhere: the planner simply uses the best available learning order).
...
Compiler-style pipeline: `Knowledge Graph → Curriculum Designer (ZOLTRAAK) → Curriculum Artifact (stored when you apply one; otherwise auto-drafted on the fly — the learner never has to approve a curriculum) → Teaching flavors`. Two fundamentally different activities, two prompt architectures:
